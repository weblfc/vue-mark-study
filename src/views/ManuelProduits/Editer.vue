      ...
      <div class="Contenuitem">
        <div class="topinput">
          <div>
            ...
            <span>产品分类：</span>
            <a-select
              v-model="productAll.productName"
        ...
        <div class="modelsave addfoot">
          ...
          <div @click="modelAnnuler"><i></i> 删除</div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    modelAnnuler() {
      if (confirm("是否删除")) {
        this.axios({
          url: "/api/manual/delete/" + this.ProduitsId,
          method: "get",
        })
          .then((result) => {
            if (result.data.rlt == 0) {
              this.success();
    ...
    companyId(value) {
      // 如果选择的是天津天元海，companyId就等于1，如果选择的是美国秦兰，companyId=2
      this.company.forEach((ele) => {
        if (value == ele.companyName) {
    ...
    init() {
      ...
      this.axios({
        ...
        .then((result) => {
          ...
          // this.companyId = result.data.datas.companyId;
          // 循环遍历，获得产品名称
        ...
        this.axios({
          ...
          .then((result) => {
            ...
            this.modelList = this.productAll.modelList;
            this.configList = result.data.datas.configList;
            this.companyName.push(result.data.datas.companyName);
            ...
            this.fileList.map((ele) => {
              ele.url = `/${ele.filePath}/${ele.storeName}`;
              ele.uid = ele.id;
              ele.status = "done";
              ele.name = ele.fullName;
              ele.response = { rlt: 0, datas: JSON.parse(JSON.stringify(ele)) };
              return ele;
            });
          })
          .catch((err) => {
  ...
  activated() {
    if (this.ProduitsId != this.$route.params.id) {
